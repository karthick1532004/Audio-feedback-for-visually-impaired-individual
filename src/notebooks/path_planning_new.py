...
sys.path.append(r'/media/boobalan/New Volume/college/FIVETH SEM/system thinking/system_thinking_1/system_thinking')

from models import net  # Assuming 'net' is the class or function you want to import from 'models.py'
import torch
from torch.autograd import Variable

def binary_image(img):
    # Pre-processing and post-processing constants #
    CMAP = np.load(r'/media/boobalan/New Volume/college/FIVETH SEM/system thinking/system_thinking_1/system_thinking/src/cmap_nyud.npy')
    ...
    ckpt = torch.load(r'/media/boobalan/New Volume/college/FIVETH SEM/system thinking/system_thinking_1/system_thinking/weights/ExpNYUD_joint.ckpt')
    ...
    img_path = cv2.resize(img, (640, 480))
    ...
    mapping = np.full_like(img, (255, 255, 255), dtype=np.uint8)

    # Set the floor area in the mapping to black color
    mapping[floor_mask] = (0, 0, 0)
    mapping_new = cv2.dilate(mapping,np.ones((7,7),np.uint8),iterations=5)
    # Find contours in the binary floor mask
    contours, _ = cv2.findContours(floor_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    return mapping_new,segm
