...
floor_color = (0, 255, 0)  # Green color (BGR format)

# Create a mask for the floor region
floor_mask = (segm == floor_color).all(axis=2)

# Find the center of the floor region based on the mask
if np.any(floor_mask):
    cy, cx = np.where(floor_mask)

    # Find the middle point of the floor region
    cy_mean = int(np.mean(cy))
    cx_mean = int(np.mean(cx))

    # Draw a line in the middle of the floor region
    line_color = (255, 0, 0)  # Red color (BGR format)
    thickness = 2
    line_length = 100
    pt1 = (cx_mean - line_length // 2, cy_mean)
    pt2 = (cx_mean + line_length // 2, cy_mean)
    floor_with_line = frame.copy()
    cv2.line(floor_with_line, pt1, pt2, line_color, thickness)

    # Display the original image with the floor segment and the line in the middle
    cv2.imshow('Floor with Line', floor_with_line)
    cv2.waitKey(0)
